...
talTraits <-  tolsAll %>%
  group_by(TRAITS_NAME, CatTraits) %>%
  tally() %>%
  mutate(totSp = 343) %>%
  mutate(PercTraits = (n/343)*100)
...
## calculte means of trait per site, weighted on capture probability per site. 
## o/e calcuated by weight mean oberseved/weighted mean expected
...
bio_h_summary <- bio_h_summary %>% filter(CatTraits %in% c("Thermal Indicator", "Voltinism"))
bio_h_summary
...
supp.labs <- c(
  ...
  "Max_Wkl_Max_StreamT" = "Weekly Max",
  "Min_Wkl_Min_StreamT" = "Weekly Min",
...
for(m in 1:length(mets)) {
  ...
  T1 <- ggplot(subset(AllDataWide, BioCat == cats[c]), aes(y=predictedVals, x=Value, group = AvText, color = AvText)) +
